...
Compatible con PostgreSQL/Supabase (y SQLite en fallback de emergencia).
UUIDs almacenados como String para compatibilidad total sin extensiones.

v3.1 — Merge resuelto con soporte PostgreSQL/Supabase:
  - UsuarioSQL        (gestión de roles y autenticación)
  - PerfilDonanteSQL  (perfil extendido, preferencias e historial)
  - FamiliaSQL        (ocupación operativa del albergue)
...
class UsuarioSQL(Base):
    ...
    Preparada para migración a PostgreSQL/Supabase:
      - UUIDs como PK en formato String (compatible sin extensiones)
    ...
    rol            = Column(SAEnum(RolUsuario, name="rolusuario"), nullable=False, default=RolUsuario.donante)
...
class FamiliaSQL(Base):
    ...
    dias_estancia_est      = Column(Integer, nullable=False, default=7)
    ...
    necesidades_especiales = Column(Text, nullable=True)
